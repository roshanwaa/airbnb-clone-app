...
export const PhotosUploader = ({ addedPhotos, onChange }) => {
  ...
  const removePhotosHandler = (event, fileName) => {
    event.preventDefault();
    onChange([...addedPhotos.filter((photo) => photo !== fileName)]);
  };
  const selectMainPhoto = (event, fileName) => {
    event.preventDefault();
    // const editPhotos = addedPhotos.filter((photo) => photo !== fileName);
    const newEditPhotos = [
      fileName,
      ...addedPhotos.filter((photo) => photo !== fileName),
    ];
    onChange(newEditPhotos);
  };
      ...
      <div className="grid gap-4 grid-cols-3 md:grid-cols-4 lg:grid-cols-6 mt-2 ">
        {addedPhotos.length > 0 &&
          addedPhotos.map((link) => (
            <div
              className="h-32 flex relative duration-300 hover:scale-105"
              key={link}>
              <img
              ...
              <button
                onClick={(event) => {
                  removePhotosHandler(event, link);
                }}
                className="absolute bottom-2 right-2 p-1 text-white bg-black bg-opacity-50 rounded-xl cursor-pointer hover:bg-rose-500">
                <svg
              ...
              <button
                ...
                className="absolute bottom-2 left-2 p-1 text-white bg-black bg-opacity-50 rounded-xl cursor-pointer hover:bg-rose-500">
                {link === addedPhotos[0] && (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
